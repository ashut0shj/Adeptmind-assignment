...
def scrape_page_elements(url):
    ...
    # Extract header (div with class 'bs-header')
    ...
    if product_list:
        for item in product_list.find_all('li', class_='product-item', recursive=False):
            ...
            # Rating
            rating_tag = item.find('span', class_='plp-rating')
            product['rating'] = rating_tag.get_text(strip=True) if rating_tag else None
            # Number of Reviews
            reviews_tag = item.find('span', class_='plp-rating-count')
            product['num_reviews'] = reviews_tag.get_text(strip=True) if reviews_tag else None
